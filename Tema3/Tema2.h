...
class Tema2 : public SimpleScene
{
	...
	private:
		...
		void UpdateCollectibles(float deltaTimeSeconds);
		void UpdateFences(float deltaTimeSeconds);
		void UpdateObstacles(float deltaTimeSeconds);
		void UpdateLights(float deltaTimeSeconds);
		void GenerateLight(int col, int i, int z);

		void RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 &modelMatrix, Texture2D* texture, bool use_light, glm::vec3 color = glm::vec3(0, 0, 0));
	...
	protected:
		...
		float score = 0;
		float score_max = 90;
		glm::vec3 score_bar_position = glm::vec3(0.5, 0.9, 0);
		...
		glm::vec3 light_colors_arr[6] = {
			glm::vec3(1, 0, 0), // red
			glm::vec3(0, 0.8, 0), // green
			glm::vec3(0, 0, 1), // blue
			glm::vec3(0.9, 0.9, 0), // yellow
			glm::vec3(1, 0, 1), // purple
			glm::vec3(0, 1, 1) // turqoise
		};
		...
		// Decorative fence:
		float fence_height1 = 1.5, fence_width1 = 0.2;
		float fence_height2 = 0.2, fence_length2 = 3, fence_width2 = 0.2;

		float fences_oz[2][20];
		int lamp_type[2][20];
		int fences_count[2];
		...
		// Collectible:
		float collectible_oz[4][20];
		int collectible_count[4] = { 0, 0, 0, 0 };
		float collectible_angle[4][20];

		float next_collectible_in = 2;
		float last_collectible_in = 0;

		// Obstacles:
		float obstacle_oz[4][20];
		int obstacle_count[4] = { 0, 0, 0, 0 };
		float obstacle_height = 1.2, obstacle_width = 0.2;

		float next_obstacle_in = 2;
		float last_obstacle_in = 0;

		// Lighting:
		glm::vec3 light_positions[20];
		glm::vec3 light_color[20];
		int is_spotlight[20];
		int light_count = 0;
